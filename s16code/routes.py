...
async def channel_message(
    ...
    """Run any GLC channel envelope through the same S16 live graph."""
    expected = os.getenv("S16_CHANNEL_BRIDGE_TOKEN", "").strip()
    supplied = (authorization or "").removeprefix("Bearer ").strip()
    if not expected:
        raise HTTPException(503, "S16_CHANNEL_BRIDGE_TOKEN is not configured")
    ...
    from s16code.events import EventEnvelope
...
"""
    ...
    configured_authority = {
        item.strip()
        for item in os.getenv("S16_CHANNEL_ALLOWED_SIDE_EFFECTS", "").split(",")
        if item.strip()
    }
    ...
    try:
        ...
        result = await request.app.state.runtime.run(
            prompt=prompt,
            scope=MemoryScope(
                os.getenv("S16_CHANNEL_TENANT", "local"),
                "channels",
                principal_id,
                "s16-channel-agent",
            ),
